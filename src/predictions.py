...
def get_available_matches(id_usuario: int) -> pd.DataFrame:
    ...
    sql = """
        SELECT
            ...
            datetime(date(p.fecha_hora_partido)) AS fecha_cierre_automatica,
            ...
            CASE
                WHEN datetime('now', 'localtime') < datetime(vp.fecha_apertura)
                    THEN 'No disponible'

                WHEN datetime('now', 'localtime') >= datetime(date(p.fecha_hora_partido))
                    THEN 'Cerrada'
            ...
            CASE
                WHEN datetime('now', 'localtime') >= datetime(vp.fecha_apertura)
                 AND datetime('now', 'localtime') < datetime(date(p.fecha_hora_partido))
        ...
        WHERE datetime('now', 'localtime') >= datetime(vp.fecha_apertura)

        ORDER BY
            datetime(p.fecha_hora_partido) ASC,
            p.id_partido ASC
    """

    return fetch_df(sql, (id_usuario,))
...
def can_create_prediction(id_usuario: int, id_partido: int) -> tuple[bool, str]:
    ...
    df = fetch_df(sql, (id_usuario, id_partido))
...
def save_prediction(
    ...
    puede_guardar, mensaje = can_create_prediction(id_usuario, id_partido)
    ...
    try:
        execute(sql, (
            id_usuario,
            id_partido,
            int(goles_local_predicho),
            int(goles_visitante_predicho),
        ))
...
def ensure_default_predictions(id_usuario: int) -> None:
    ...
    sql = """
        ...
        INNER JOIN ventanas_prediccion vp
            ON p.id_partido = vp.id_partido
        WHERE datetime('now', 'localtime') >= datetime(date(p.fecha_hora_partido))
          AND NOT EXISTS (
                SELECT 1
...
def lock_expired_predictions() -> None:
    ...
    sql = """
        ...
        WHERE id_partido IN (
            SELECT id_partido
            FROM partidos
            WHERE datetime('now', 'localtime') >= datetime(date(fecha_hora_partido))
        )
    """
